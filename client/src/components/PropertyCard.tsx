        ...
        <div className="absolute bottom-0 w-full">
          ...
          <div className="p-4 flex justify-between items-end border-x border-b border-white/20 bg-black/45 backdrop-blur-xl text-white shadow-sm">
            <div>
              ...
              <div className="font-body-md text-body-md text-white/80 line-clamp-1">
                {listing.location}
              </div>
            </div>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="p-3 text-center border-r border-white/20 last:border-r-0">
      <span className="block font-headline-md text-headline-md leading-none mb-1">
        {value}
      </span>
      <span className="font-label-sm text-label-sm text-white/80 uppercase">
        {label}
      </span>
    </div>
  );
