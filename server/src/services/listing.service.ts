...
export async function getAllListings() {
    const result = await pool.query("SELECT * FROM listings WHERE status='approved'");
    return result.rows;
}

export async function getListingById(id: string) {
    const result = await pool.query("SELECT * FROM listings WHERE status='approved' AND id=$1", [id]);
...
export async function createListing(ownerId: number, data: CreateListingInput) {
    ...
    try {
        ...
        const insert = await client.query(
            ...
            [title, description, latitude, longitude, bedroom_count, bathroom_count, on_which_floor, area_id, ownerId]
        );
...
export async function updateListing(id: string, ownerId: number, data: UpdateListingInput) {
...
export async function deleteListing(id: string, ownerId: number) {
    ...
    if (listing.rows.length === 0) throw new AppError(404, 'listing not found');
    if (listing.rows[0].owner_id !== ownerId) throw new AppError(403, 'not authorized');
    await pool.query("UPDATE listings SET status='unavailable' WHERE id=$1", [id]);
}
