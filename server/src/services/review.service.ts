...
export async function createReview(tenantId: number, data: CreateReviewInput) {
  ...
  const contract = await pool.query(
    "SELECT 1 FROM contracts WHERE id=$1 AND tenant_id=$2 AND status='completed'",
    [contract_id, tenantId],
  );
  if (contract.rows.length === 0)
    throw new AppError(403, "this contract doesnt exist or is not completed");
  ...
  await pool.query(
    "INSERT INTO reviews (contract_id,rating,description) VALUES ($1,$2,$3)",
    [contract_id, rating, description],
  );
}
