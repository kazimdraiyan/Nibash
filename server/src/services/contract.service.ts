...
export async function createContract(
  ...
  if (listing.rows.length === 0)
    throw new AppError(403, "this listing isnt yours");

  const application = await pool.query(
    "SELECT 1 FROM applies WHERE tenant_id=$1 AND listing_id=$2 AND status='pending'",
    [tenant_id, listing_id],
  );
  if (application.rows.length === 0)
    throw new AppError(403, "this application isnt pending");

  const activeContract = await pool.query(
    "SELECT 1 FROM contracts WHERE listing_id=$1 AND status IN ('proposed','signed')",
    [listing_id],
  );
  if (activeContract.rows.length > 0)
    throw new AppError(
      403,
      "there is already an active contract for this listing",
    );
  ...
  try {
    ...
    const insertAgreement = await client.query(
    ...
    await client.query(
      "UPDATE applies SET status='approved' WHERE tenant_id=$1 AND listing_id=$2",
      [tenant_id, listing_id],
    );
    await client.query(
...
export async function getContractById(id: string, userId: number) {
  ...
  if (
    contract.rows[0].owner_id !== userId &&
    contract.rows[0].tenant_id !== userId
  ) {
...
export async function signContract(id: string, userId: number) {
  ...
  if (contract.rows.length === 0) throw new AppError(404, "contract not found");
  if (contract.rows[0].tenant_id !== userId)
    throw new AppError(403, "not authorized");
  if (contract.rows[0].status !== "proposed")
    throw new AppError(400, "contract status cannot be updated");
