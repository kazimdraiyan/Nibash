...
export async function getForListing(req: Request, res: Response) {
    if (!req.user) { res.status(401).json({ error: 'unauthorized' }); return; }
    const applications = await applicationService.getApplicationsForListing(req.user.id, req.params.listingId);
    res.json({ applications });
}
...
export async function reject(req: Request, res: Response) {
    ...
    if (!result.success) { res.status(400).json({ error: result.error.issues[0].message }); return; }
    await applicationService.rejectApplication(req.user.id, req.params.listingId, req.params.tenantId);
